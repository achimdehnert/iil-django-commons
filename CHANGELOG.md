...
### Fixed (2026-03-11)
- `BaseTask` now inherits from `celery.Task` (was standalone, unusable)
- `email/service.py`: `os.environ` → `django.conf.settings` for `RESEND_API_KEY` (ADR-045)
- `health/views.py`: added `@csrf_exempt` + `@require_GET`, latency tracking, `/healthz/` alias
- `cache/decorators.py`: `md5(usedforsecurity=False)`
- Version mismatch fixed: `__init__.py` now matches `pyproject.toml` (0.3.0)
- `requires-python` relaxed to `>=3.10`
- Removed unused `structlog` dependency
- Tests: deduplicated `rf` fixture into `conftest.py`, fixed `celery.Task.request` property
